...
public class VendusService(IConfiguration config, ILogger<VendusService> logger) : IVendusService
{
    ...
    private async Task<string> GetPublisherFromProduct(Product product)
    {
        ...
        var vendusObj = JsonConvert.DeserializeObject(response);
        var type = vendusObj.GetType();
        
        var publisherNames = string.Empty;
        if (type == typeof(Newtonsoft.Json.Linq.JArray))
        {
            var publishers =
                JsonConvert.DeserializeObject<List<Publisher>>(response);
            
            foreach (var publisher in publishers)
            {
                publisherNames += publisher.Title + ", ";
            }
            
            if(publisherNames.Length > 0)
                publisherNames = publisherNames.Substring(0, publisherNames.Length -(publisherNames.Length - publisherNames.LastIndexOf(',')));
        }
        else
        {
            var publishers =
            ...
            publisherNames = publishers.Title;
        }
