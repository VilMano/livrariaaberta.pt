...
public class ImageService(IConfiguration config, IBookService bookService, ILogger<ImageService> logger) : IImageService
{
    ...
    public async Task<ResultWrapper<byte[]>> HandleImageCreation(IFormFile file)
    {
        ...
        {
            book.Data.CoverPicture = $"{isbn}/{isbn}.{fileType}";
            await bookService.UpdateBook(book.Data);
        }
